...
import { SectionCard } from "@/components/platform-sections";
import { formatRange, formatTime, minutesUntil, type Room } from "@/lib/platform";

export function RoomsPageClient({ rooms }: { rooms: Room[] }) {
  const [selectedRoomId, setSelectedRoomId] = useState(rooms[0]?.id ?? "");
  const selectedRoom = rooms.find((room) => room.id === selectedRoomId) ?? rooms[0];

  if (!selectedRoom) {
    return <SectionCard eyebrow="Room inventory" title="Connected spaces">No rooms configured yet.</SectionCard>;
  }
      ...
      <SectionCard eyebrow="Room detail" title={selectedRoom.name}>
        <div className="stack-gap">
          ...
          <div className="timeline-list">
            {selectedRoom.upcomingMeetings.length ? selectedRoom.upcomingMeetings.map((meeting) => (
            ...
            )) : <div className="empty-state">Nothing else booked soon. A rare moment of calendar mercy.</div>}
          </div>
        </div>
      </SectionCard>
    </div>
